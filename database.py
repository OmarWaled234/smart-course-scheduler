...
def add_task(
    ...
    with get_connection() as conn:
        conn.execute(
            """
            INSERT INTO tasks
                (title, course_id, task_type, due_date, estimated_hours, priority, completed)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (title.strip(), course_id, task_type, due_date, estimated_hours, priority, int(completed)),
        )
    ...
    with get_connection() as conn:
        conn.execute(
            ...
            SET title = ?, course_id = ?, task_type = ?, due_date = ?,
                estimated_hours = ?, priority = ?, completed = ?
            WHERE id = ?
            """,
            (title.strip(), course_id, task_type, due_date, estimated_hours, priority, int(completed), task_id),
        )
...
def add_study_block(day_of_week: str, start_time: str, end_time: str) -> None:
    with get_connection() as conn:
        conn.execute(
            "INSERT INTO study_blocks (day_of_week, start_time, end_time) VALUES (?, ?, ?)",
            (day_of_week, start_time, end_time),
        )


def update_study_block(block_id: int, day_of_week: str, start_time: str, end_time: str) -> None:
    with get_connection() as conn:
        conn.execute(
            "UPDATE study_blocks SET day_of_week = ?, start_time = ?, end_time = ? WHERE id = ?",
...
def add_demo_data() -> None:
    ...
    with get_connection() as conn:
        ...
        tasks = [
            ("Integration Practice Set", course_ids["Calculus II"], "assignment", "2026-05-08", 3.5, "high", 0),
            ("Python DataFrames Lab", course_ids["Computer Science"], "project", "2026-05-12", 5.0, "high", 0),
            ("Poetry Response Draft", course_ids["English Literature"], "assignment", "2026-05-10", 2.0, "medium", 0),
            ("Cell Biology Quiz", course_ids["Biology"], "quiz", "2026-05-07", 1.5, "high", 0),
            ("Read Chapter 9", course_ids["Biology"], "reading", "2026-05-14", 2.0, "low", 0),
            ("Old Discussion Post", course_ids["English Literature"], "other", "2026-05-01", 1.0, "medium", 0),
            ("Completed Recursion Notes", course_ids["Computer Science"], "reading", "2026-05-06", 1.0, "low", 1),
        ]
        conn.executemany(
            """
            INSERT INTO tasks
